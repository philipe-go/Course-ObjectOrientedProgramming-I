...
namespace ShapesDataLayer
{
    ...
    class Rectangles : Squares
    {
        ...
        public void Draw(PaintEventArgs e)
        {
            ...
            Color bordercolor = Convert.ToString(this.shapeBorderColor);
            Color brushcolor = Convert.ToString(this.shapeFillColor);
            Pen myPen = new Pen(bordercolor, 2);
            Brush myBrush = new SolidBrush(brushcolor);

            g.DrawRectangle(myPen, this.xCoord, this.yCoord, Convert.ToInt32(this.width), Convert.ToInt32(this.length));
            myPen.Dispose();
            myBrush.Dispose();
            e.Graphics.Dispose();
        }
